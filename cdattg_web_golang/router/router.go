	...
	api := r.Group("/api")
	{
		...
		protected.Use(middleware.AuthMiddleware())
		{
			...
			lms := protected.Group("/lms")
			{
				...
				lms.GET("/auditoria/fichas/:fichaId/personas", middleware.RequireSuperAdmin(), lmsAuditoriaHandler.ListarFicha)
				lms.GET("/auditoria/personas/:personaId", middleware.RequireSuperAdmin(), lmsAuditoriaHandler.Persona)
				lms.GET("/auditoria/personas/:personaId/tipos/:tipo", middleware.RequireSuperAdmin(), lmsAuditoriaHandler.Tipo)
				lms.GET("/aulas/:fichaId", middleware.RequirePermission("lms", permEntrarAulaLMS), lmsHandler.GetAula)
				lms.GET("/aulas/:fichaId/calificaciones", middleware.RequirePermission("lms", permEntrarAulaLMS), lmsHandler.HistorialCalificaciones)
