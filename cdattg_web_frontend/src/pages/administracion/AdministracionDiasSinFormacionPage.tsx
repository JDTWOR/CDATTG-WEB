...
export function AdministracionDiasSinFormacionPage() {
  ...
  const itemsFiltrados = useMemo(() => {
    if (filterSedeIds.length === 0) return items;
    const filtro = new Set(filterSedeIds);
    return items.filter((item) => filtro.has(item.sede_id));
  }, [filterSedeIds, items]);
  ...
  const guardar = async () => {
    ...
    try {
      ...
      await Promise.all(
        form.sede_ids.map((sede_id) => apiService.createDiaSinFormacion({ ...payload, sede_id })),
      );
      setForm(emptyForm());
      await load();
    } catch (err: unknown) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            <SelectSearchMulti
              ...
              options={sedeOptions}
              value={form.sede_ids}
              onChange={(sede_ids) => setForm({ ...form, sede_ids })}
      ...
      <div className="card p-4">
        ...
        {cuerpoTablaRegistros(loading, itemsFiltrados, eliminar)}
      </div>
    </div>
  );
}
